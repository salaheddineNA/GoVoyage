...
<body>
        ...
        <div class="max-w-8xl mx-auto px-6 py-16">
            <div class="text-center mb-12">
                <h1 class="text-4xl md:text-5xl font-bold m-6">
                    Trouvez votre vol parfait
                </h1>
                <p class="text-xl text-blue-100">
                    Comparez les prix et réservez votre prochain voyage en quelques clics
                </p>
            </div>
            
            <!-- Search Form -->
            <form action="{{ route('flight') }}" method="GET" class="bg-white rounded-2xl shadow-2xl p-6 md:p-8">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    ...
                    <div>
                        ...
                        <input type="date" 
                               name="departure_date" 
                               value="{{ request()->query('departure_date') }}"
                               class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900">
                    </div>
                </div>
    ...
    <section class="relative bg-[#0c47bc]  pt-20 pb-32">
            <div class="relative z-10 flex flex-col items-center justify-center text-center px-4">
                ...
                <button onclick="window.location='{{ route('support') }}'"
                    class="px-8 py-3 border-2 border-white text-white font-semibold rounded hover:bg-white hover:text-blue-500 transition-all duration-300">
                    NOUS CONTACTER
                </button>
            </div>
        ...
        <div class="max-w-7xl mx-auto px-6">
            ...
            <div class="mb-8">
                <h2 class="text-3xl font-bold text-gray-900">
                    {{ request()->query('departure') || request()->query('arrival') || request()->query('max_price') || request()->query('direct') || request()->query('wifi') ? 'Résultats de recherche' : 'Tous les vols disponibles' }}
                </h2>
                <p class="text-gray-600 mt-2">
                    {{ $flights->count() }} vol{{ $flights->count() > 1 ? 's' : '' }} trouvé{{ $flights->count() > 1 ? 's' : '' }}
                    @if(request()->query('departure') || request()->query('arrival') || request()->query('max_price') || request()->query('direct') || request()->query('wifi'))
                        <a href="{{ route('flight') }}" class="text-blue-600 hover:text-blue-800 ml-2">
                            ← Effacer les filtres
                        </a>
                    @endif
                </p>
            ...
            <div class="mb-6 flex flex-wrap gap-2">
                ...
                @endif
                @if(request()->query('arrival'))
                <span class="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm">
                    Arrivée: {{ request()->query('arrival') }}
                </span>
                @endif
                ...
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300">
                    ...
                    <div class="p-6">
                        ...
                        <div class="flex items-center justify-between mb-4">
                            <div>
                                ...
                                <div class="flex items-center space-x-2">
                                    <span class="text-gray-400 line-through">{{ number_format($flight->oldprice, 2) }} MAD</span>
                                    <span class="text-2xl font-bold text-green-600">{{ number_format($flight->newprice, 2) }} MAD</span>
                                </div>
                                <span class="text-xs text-red-600 font-medium">-{{ $flight->percentage }}%</span>
                                @else
                        ...
                        <a href="{{ route('bookings.create', $flight->id) }}" 
                           class="w-full block text-center bg-gradient-to-r from-blue-600 to-indigo-600 text-white py-3 px-4 rounded-xl hover:from-blue-500 hover:to-indigo-500 transition font-medium transform hover:scale-105">
                            🎫 Réserver ce vol
                        </a>
                    </div>
                </div>
            ...
            <div class="text-center py-16">
                ...
                <p class="text-gray-500 mb-6">Essayez d'ajuster vos critères de recherche.</p>
                <a href="{{ route('flight') }}" class="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700">
                    Voir tous les vols
                </a>
            </div>
            @endif
